...
4. Added a pseudo-element in the summary styles (rather than touching the link-tree markup)
   so each collapsed card now exposes a chevron that indicates it is expandable without
   changing the existing DOM shape.
